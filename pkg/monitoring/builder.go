...
func BuildServiceMonitor(rev *v1.IstioRevision, opts Options) *monitoringv1.ServiceMonitor {
	...
	sm.SetGroupVersionKind(monitoringGV(opts).WithKind("ServiceMonitor"))
	return sm
}
...
func BuildPodMonitor(rev *v1.IstioRevision, namespace string, opts Options) *monitoringv1.PodMonitor {
	...
	pm.SetGroupVersionKind(monitoringGV(opts).WithKind("PodMonitor"))
	return pm
}
...
func monitoringGV(opts Options) schema.GroupVersion {
	if opts.MonitoringGV != (schema.GroupVersion{}) {
		return opts.MonitoringGV
	}
