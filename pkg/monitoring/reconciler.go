...
const (
	...
	// DefaultMonitoredByLabel is the label key used so a Prometheus stack selects these monitors.
	// DefaultMonitoredByValue is hardcoded for the interim Istio.spec.monitoring.enabled flow;
...
type Options struct {
	...
	MonitoringGV  schema.GroupVersion
...
func ReconcileRevision(ctx context.Context, c client.Client, rev *v1.IstioRevision, opts Options) error {
	...
	opts.MonitoringGV = resolveMonitoringGV(opts.RESTMapper, opts.MonitoringGV, opts.ForCOO)
	log.V(1).Info("Using monitoring API group", "group", opts.MonitoringGV.Group)
...
func reconcileServiceMonitor(ctx context.Context, c client.Client, rev *v1.IstioRevision, opts Options) error {
	...
	existing.SetGroupVersionKind(opts.MonitoringGV.WithKind("ServiceMonitor"))
...
func reconcilePodMonitorInNamespace(ctx context.Context, c client.Client, rev *v1.IstioRevision, namespace string, opts Options) error {
	...
	existing.SetGroupVersionKind(opts.MonitoringGV.WithKind("PodMonitor"))
